...
class SignalPreProcessing:
    ...
    def calc_peaks(self, crop_figure):
        ...
        """
        partial_peaks, partial_properties = find_peaks(crop_figure, prominence=self.prominence, height=self.baseline)
        partial_negative_peaks, partial_negative_properties = find_peaks(-crop_figure, prominence=10, height=self.neg_baseline)
    ...
    def calc_width_distribution(self, crop_figure, col, global_label):
        ...
        """
        plt.close()
        total_width = np.concatenate((self.p_width[0] / 10, self.n_width[0] / 10), axis=None)
        bins = np.arange(np.floor(total_width.min()), np.ceil(total_width.max()), 1)
        ...
        plt.plot(bins, y, 'r--', linewidth=2, color="black", alpha=0.8, label=global_label)
        plt.xlabel('Width (s)')
        plt.ylabel('Probability')
        plt.title(r'$\mu=%.3f,\ \sigma=%.3f$' % (mu_w, sigma_w))
        plt.legend()
        plt.savefig(f'statistics_analysis/distribution_width/{col}.png', transparent=True)
        ...
        plt.title(r'$\mu=%.3f,\ \sigma=%.3f$' % (mu_w, sigma_w), fontsize=26, fontweight='bold')
        plt.xticks(fontsize=24)
        plt.yticks(fontsize=24)
        plt.legend(prop={'size': 22})
        fig.savefig(f'statistics_analysis/distribution_width/{col}_big.png', transparent=True)
        plt.close()
    ...
    def calc_height_distribution(self, crop_figure, col, global_label):
        ...
        """
        plt.close()
        total_v1 = np.concatenate((self.p_properties["prominences"], self.n_properties["prominences"]), axis=None)
        ...
        plt.legend()
        plt.savefig(f'statistics_analysis/distribution_height/{col}.png', transparent=True)

        fig = plt.figure(figsize=(12, 9), dpi=300)
        plt.hist(total_v1, bins, density=1, facecolor='green', alpha=0.75)
        plt.plot(bins, y, 'r--', linewidth=2, color="black", alpha=0.5, label=global_label)
        plt.xlabel('Height (\u03BCV)', fontsize=28, fontweight='bold')
        plt.ylabel('Probability', fontsize=28, fontweight='bold')
        plt.title(r'$\mu=%.3f,\ \sigma=%.3f$' % (mu, sigma), fontsize=28, fontweight='bold')
        plt.xticks(fontsize=24)
        plt.yticks(fontsize=24)
        plt.legend(prop={'size': 22})
        fig.savefig(f'statistics_analysis/distribution_height/{col}_big.png', transparent=True)
        plt.close()
    ...
    def draw_height_time(self, crop_figure, col, global_label):
        ...
        """
        plt.close()
        total_p = np.concatenate((self.p_properties["prominences"], self.n_properties["prominences"]))
        total_count_p = np.concatenate((self.peaks_data, self.neg_peaks_data))

        plt.plot(np.array(total_count_p) / 864000, total_p, color="black", alpha=0.8, label=global_label)
        plt.ylabel('Height (\u03BCV)')
        plt.xlabel('Time (day)')
        plt.legend()
        plt.savefig(f'statistics_analysis/t_height/{col}.png', transparent=False)
        ...
        fig.savefig(f'statistics_analysis/t_height/{col}_big.png', transparent=True)
        plt.close()
    ...
    def draw_width_time(self, crop_figure, col, global_label):
        ...
        plt.plot(np.array(total_count_w) / 864000, total_w / 10, color="black", alpha=0.8, label=global_label)
        plt.ylabel('Width (s)')
        plt.xlabel('Time (day)')
        plt.legend()
        plt.savefig(f'statistics_analysis/t_width/{col}.png')
        ...
        fig.savefig(f'statistics_analysis/t_width/{col}_big.png', transparent=True)
        plt.close()
    ...
    def draw_height_width(self, crop_figure, col, global_label):
        ...
        plt.scatter(total_w / 10, total_p, color="black", alpha=0.8, label=global_label)
        plt.ylabel('Height (\u03BCV)')
        plt.xlabel('Width (s)')
        plt.legend()
        plt.savefig(f'statistics_analysis/height_width/{col}.png', transparent=True)
        ...
        plt.yticks(fontsize=24)
        plt.legend(prop={'size': 22})
        fig.savefig(f'statistics_analysis/height_width/{col}_big.png', transparent=True)
        plt.close()
...
def main():
    ...
    global_label = '$\it{Pleurotus}$ $\it{eryngii}$ 60 mm petri dish'

    # Load and concatenate data from the specified file
    file_path = 'B1_D1.csv'  # data input
    ...
    plt.plot(x_axis / 864000, total_file_data, color="black", alpha=0.8, label=global_label)
    plt.xlabel('Time (day)', fontsize=24, fontweight='bold')
    plt.ylabel('Action Potentials (\u03BCV)', fontsize=24, fontweight='bold')
    plt.legend()
    plt.savefig(f'statistics_analysis/macro_v_time/t_mv_{col}.png', transparent=True)
    plt.close()
    ...
    fig.savefig(f'statistics_analysis/macro_v_time/t_mv_{col}_big.png', transparent=True)
    plt.close()

    pos_p, neg_p, pos_prop, neg_prop = sig_pp.calc_peaks(total_file_data)
    pos_w, neg_w, pos_x_w, neg_x_w = sig_pp.calc_width(total_file_data, pos_p, neg_p)
    ...
    for x in range(int(len(total_file_data) / sig_pp.crop_interval)):
        crop_figure = total_file_data[sig_pp.crop_cnt:sig_pp.crop_cnt + sig_pp.crop_interval]
    ...
    plt.text(230, 0.04, f'mean = {np.mean(total_freq):.3f} , std = {np.std(total_freq):.3f}', fontsize=12, horizontalalignment='center', verticalalignment='center')
    ...
    plt.legend()
    plt.savefig(f'statistics_analysis/peak_per_min/col_{col}.png', transparent=True)
    plt.close()

    fig = plt.figure(figsize=(12, 9), dpi=300)
    plt.text(170, 0.035, f'mean = {np.mean(total_freq):.3f} , std = {np.std(total_freq):.3f}', fontsize=22, horizontalalignment='center', verticalalignment='center')
    plt.plot(total_freq, color="black", alpha=0.8, label=global_label)
    ...
    fig.savefig(f'statistics_analysis/peak_per_min/col_{col}_big.png', transparent=True)
    plt.close()
